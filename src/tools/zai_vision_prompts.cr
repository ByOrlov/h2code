module H2code
  module Tools
    # System prompts and tool descriptions for the built-in Z.AI Vision
    # tools — a Crystal port of the official `@z_ai/mcp-server` npm package
    # v0.1.5 (Apache-2.0, (c) Z.AI, https://docs.z.ai/devpack/mcp/vision-mcp-server).
    # The texts are kept verbatim so tool behaviour matches the MCP server.
    module ZaiVisionPrompts
      DATA_VIZ_ANALYSIS_PROMPT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      DIAGRAM_UNDERSTANDING_PROMPT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      ERROR_DIAGNOSIS_PROMPT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      GENERAL_IMAGE_ANALYSIS_PROMPT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      UI_DIFF_CHECK_PROMPT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      UI_TO_ARTIFACT_PROMPTS_CODE = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      UI_TO_ARTIFACT_PROMPTS_PROMPT = <<-PROMPT_EOF
You are an expert at reverse-engineering user interfaces and crafting precise, actionable prompts that could guide another AI to recreate them.
...
PROMPT_EOF
      ...
      UI_TO_ARTIFACT_PROMPTS_SPEC = <<-PROMPT_EOF
You are a design systems architect with extensive experience documenting user interfaces for development teams.
...
PROMPT_EOF
      ...
      UI_TO_ARTIFACT_PROMPTS_DESCRIPTION = <<-PROMPT_EOF
You are a UX writer and interface analyst who excels at describing user interfaces in clear, natural language.
...
PROMPT_EOF
      ...
      UI_TO_ARTIFACT_PROMPTS = {
        "code"        => UI_TO_ARTIFACT_PROMPTS_CODE,
        "prompt"      => UI_TO_ARTIFACT_PROMPTS_PROMPT,
        "spec"        => UI_TO_ARTIFACT_PROMPTS_SPEC,
        "description" => UI_TO_ARTIFACT_PROMPTS_DESCRIPTION,
      }
      TOOL_DESCRIPTION_ANALYZE_DATA_VISUALIZATION = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_ANALYZE_IMAGE = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_ANALYZE_VIDEO = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_DIAGNOSE_ERROR_SCREENSHOT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_EXTRACT_TEXT_FROM_SCREENSHOT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_UI_DIFF_CHECK = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_UI_TO_ARTIFACT = <<-PROMPT_EOF
...
PROMPT_EOF
      ...
      TOOL_DESCRIPTION_UNDERSTAND_TECHNICAL_DIAGRAM = <<-PROMPT_EOF

  ...
  describe Worktree do
    describe "encode_project_path" do
      ...
      it "encodes a Windows drive path with a lower-cased drive letter" do
        Worktree.encode_project_path("C:\\Users\\oleg\\project1").should eq("c/Users/oleg/project1")
      end
    ...
    describe "create" do
      it "creates a worktree branched from the current branch" do
        with_git_repo do |repo, home|
          ...
          # Branched from the current branch (main), not master:
          wt_git(repo, "rev-parse", "h2code-sess01").should eq(
            wt_git(repo, "rev-parse", "main"))
          Worktree.worktree?(dest).should be_true
    ...
    describe "merged state and clean" do
      it "reports dirty/merged state and clean removes merged worktrees" do
        with_git_repo do |repo, home|
          result = Worktree.create(repo, "sess03", home)
          dest = result.path.not_nil!

          # Uncommitted work in the worktree: dirty. (The branch tip still
          # equals main's tip, so it trivially counts as merged — the dirty
          # check is what protects uncommitted files from removal.)
          File.write(File.join(dest, "b.txt"), "two\n")
          Worktree.dirty?(dest).should be_true

          # Commit in the worktree branch and merge it back: clean + merged.
          wt_git(dest, "add", "-A")
          ...
          # clean removes the merged worktree and its branch.
      ...
      it "keeps unmerged worktrees in clean" do
        with_git_repo do |repo, home|
      ...
      it "gc removes only aged, merged worktrees" do
        with_git_repo do |repo, home|
